// The decision half of D-18: given the verification schedule and what has actually been recorded
// against each activity, which activities are due right now?
//
// Split out of verification-notifications/index.ts for the reason temperatureRules.ts gives: there
// is no Deno toolchain on the dev machine, so the edge function itself cannot be typechecked or run
// locally. Keeping these functions free of Deno APIs, network calls and Supabase means they can be,
// and scripts/test-verification-schedule.mjs does.
//
// THIS FILE HAS A TWIN at src/lib/verificationSchedule.ts, kept identical below the header. The
// duplication is deliberate - a client bundle should not pull in server code, and this file uses
// Deno-style .ts import specifiers - and it follows the precedent of limitText() being copied into
// src/lib/temperatureAlerts.ts. But that precedent copies a LABEL, and this is date arithmetic:
// drift here is silent and material, so the test bundles BOTH copies and asserts they agree.
//
// Nothing here does I/O, and nothing here reads the clock. `today` is always a yyyy-MM-dd string
// passed in by the caller - the same discipline as assess(lim, tail, now), and here it also sidesteps
// every UTC-versus-local trap: "due today" must mean the day the floor is standing in.
