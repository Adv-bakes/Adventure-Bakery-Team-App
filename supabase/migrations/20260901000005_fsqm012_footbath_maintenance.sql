-- FSQM-012 - labelled containers, foot bath maintenance, microwave, disposable utensils.
-- Four open items become three, and two arrangements move to the 13.6 exemption analysis.
--
-- Answered by the site 2026-09-01:
--   - the two Sani-512 dilutions are made up into SEPARATE, CLEARLY LABELLED CONTAINERS;
--   - the foot baths are thoroughly cleaned MONTHLY, sooner if required;
--   - there IS a microwave in the break room for heating food;
--   - break room utensils are DISPOSABLE, or personal items taken home to be washed.
--
-- THE LABELLED CONTAINERS ARE THE CONTROL, so they go in the body rather than staying a caution in
-- a commit message. 20260901000004 named the hazard of running one product at two strengths and
-- said the two must never be swapped; this says HOW that is prevented. The two solutions look
-- identical - only the label tells them apart - so the label is not housekeeping, it is the control.
--
-- THE SINK QUESTION DISSOLVED RATHER THAN BEING BOUGHT. 11.3.5.9 (iii) asks for a sink for washing
-- utensils. Nothing is washed up in the break room at all: the utensils are disposable and personal
-- items go home. The fixture has no work to do. That is a materially different answer from "a
-- handwash basin next door will serve" - which is what the previous revision was implicitly leaning
-- on and which would not have survived an auditor - so the body now says there are no utensils to
-- wash, and adds the small rule that used personal items live in the owner's locker until they
-- leave rather than sitting out in the room.
--
-- THE MONTHLY CLEAN IS NOT THE CHANGE INTERVAL, AND THE DOCUMENT DOES NOT LET THE TWO BLUR. The
-- site answered with a monthly deep clean, which is the right cadence for stripping a bath. It is
-- not how often the SOLUTION is renewed. The soil a foot bath lifts off the floor is exactly what
-- spends the quat, so a correctly mixed bath left standing a month is weaker than its label says
-- long before the month is out. The body therefore states BOTH: made up fresh at the start of each
-- production day, sooner when visibly soiled or below strip target, and stripped monthly. The daily
-- interval is written here rather than left blank - a controlled document with a hole where the
-- frequency belongs is worse than one carrying a figure the site can amend - and open item 3 (a)
-- asks the site to confirm it or set a different one, then put both on FRM-901 with a strip check.
--
-- TWO THINGS MOVE FROM "OPEN QUESTION" TO "WRITE THE JUSTIFICATION" (task 13.6): the absent utensil
-- sink, and the towel dispenser with no no-touch mechanism. Both meet what their clause protects
-- without being the fixture it names, which is exactly the case 2.4.2.1 lets a site make in
-- exchange for a written risk analysis. They are listed separately from the open items now, because
-- they are no longer questions for the site - they are paragraphs somebody has to write.
--
-- What is left: the coat rack (the last physical item), the handwash stations' construction
-- material, and the foot baths' daily interval plus label verification.
--
-- Body grows 88 -> 90 lines, still 10 Parts.
--
-- GUARDED AGAINST A STALE WRITE: asserts draft, 88 lines, and the break-room wording this
...
begin
  select status,
         ...
         (content->'procedure')::text like '%The break room does **not** have a **sink%'
    into st, lines, v4
  ...
  if lines <> 88 or not v4 then
    raise exception 'FSQM-012 is not the version this migration was written against (% procedure lines, 20260901000004 break-room wording present=%). Apply the earlier migrations, or re-derive this one if the row has been edited.', lines, v4;
  end if;
end $$;
...
begin
  select jsonb_array_length(content->'procedure') as lines,
         ...
         (content->'procedure')::text like '%Foot bath maintenance%'                  as maintenance,
         (content->'procedure')::text like '%start of each production day%'           as daily,
         (content->'procedure')::text like '%cleaned **monthly**%'                    as monthly,
         (content->'procedure')::text like '%a microwave for heating food%'           as microwave,
         (content->'procedure')::text like '%utensils provided are disposable%'       as disposable,
         (content->>'revision_history') like '%three items%'                          as open_three,
         (content->>'revision_history') like '%CARRIED INTO THE MODULE 11 EXEMPTION ANALYSIS%' as carried,
         -- replaced wording must be gone
         (content->'procedure')::text like '%The break room does **not** have a **sink%' as v4_wording,
  ...
  if r.lines <> 90 or r.parts <> 10 then
    raise exception 'FSQM-012 body wrong shape: % lines, % Parts (expected 90 / 10)', r.lines, r.parts;
  end if;
  if not (r.labelled and r.maintenance and r.daily and r.monthly and r.microwave and r.disposable) then
    raise exception 'FSQM-012 answers did not all land: labelled=%, maintenance=%, daily=%, monthly=%, microwave=%, disposable=%',
      r.labelled, r.maintenance, r.daily, r.monthly, r.microwave, r.disposable;
  end if;
  if r.v4_wording then
    raise exception 'FSQM-012 still carries the 20260901000004 break-room sink wording.';
  end if;
  if not (r.open_three and r.carried) then
    raise exception 'FSQM-012 revision history wrong: three open items=%, 13.6 carry-forward=%',
      r.open_three, r.carried;
  end if;
  if not (r.foot_strength and r.soap and r.laundering and r.walkin_rule) or r.dangling then
    raise exception 'FSQM-012 regressed an earlier migration: 1:160=%, soap=%, laundering=%, walk-in=%, Form-0010=%',
      r.foot_strength, r.soap, r.laundering, r.walkin_rule, r.dangling;
  end if;
