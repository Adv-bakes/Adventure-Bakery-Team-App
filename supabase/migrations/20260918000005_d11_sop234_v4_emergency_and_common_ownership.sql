...
Two Minor findings against this procedure, and one rule for distributors. 2.3.4.4: receipt from a non-approved supplier is acceptable only in an emergency and only after a receiving inspection is conducted and recorded — there was no route for it, so the procedure's "whenever possible" left the exception undefined. 2.3.4.5: material from another site under the same corporate ownership is held to the same specification, approval and receiving requirements as any supplier — there was no provision.
...
DISTRIBUTORS: A BRAND SUBSTITUTION IS A NEW MATERIAL. Most material arrives through Sysco and Restaurant Depot, already conditionally approvable here as recognised distributors. What a distributor's approval does not cover is the brand: when an item is out of stock the distributor substitutes, or a different brand is picked up in store, and a different cake mix can carry a different allergen profile. The new step makes the brand part of what is approved, checks it at every receipt, and holds a substitution with different allergens until it has been reviewed. It also states what stands in for a per-lot certificate of analysis, which a distributor does not provide.
...
begin
  ...
  if jsonb_array_length(p) <> 28 then
    raise exception 'procedure is % lines, expected 28.', jsonb_array_length(p);
  end if;
  ...
  -- distributor brand rule
  if p->>24 like '> %' or p->>24 not like '%named product from a named manufacturer%'
     or p::text not like '%is a new material%' or p::text not like '%may contain%' then
    raise exception 'distributor brand-substitution rule is missing.';
  end if;
