-- FSQM-012 - the foot bath solution changes on condition, not on a fixed daily interval.
--
-- 20260901000005 wrote "made up fresh at the start of each production day". The owner pushed back:
-- daily replacement is excessive, change it to when necessary or when the test strip says so. He is
-- right, and the document follows him.
--
-- WHY THE PUSHBACK IS CORRECT. SQF prescribes no change interval for a foot bath - Module 11 does
-- not require foot baths at all; they are this site's chosen control for 11.3.3's footwear
-- requirement. A bath still at strength and still visibly clean is doing its job, and dumping it on
-- a calendar spends chemical to no effect. A condition trigger is the risk-based answer the code
-- actually asks for, provided the condition is measured rather than noticed in passing.
--
-- SO THE CHECK STAYS SCHEDULED AND ONLY THE CHANGE BECOMES CONDITIONAL. "Change when necessary" on
-- its own is not a control - it decays into "change it when somebody happens to notice", which is
-- indistinguishable at audit from not managing it at all. Each bath is checked at the start of every
-- production day, visually and with a quat strip, and changed when the strip reads below target,
-- when it is visibly soiled, or when it has been diluted by water carried in or by washdown. The
-- monthly strip-down is unchanged. That gives the site what it asked for without giving up the
-- evidence.
--
-- *** THE STRIP CANNOT READ THIS BATH, AND THAT IS A NEW REQUIREMENT THE DECISION CREATES. ***
-- Relying on a test strip means the strip has to be able to read the solution. At 1:160 the foot
-- bath runs at roughly three times the 1:512 food-contact strength - about 640 ppm quat against
-- 200 - which is off the top of the 0-400 ppm strips the SSOPs use on equipment. Those strips
-- saturate and read high whatever the true value is, so a checker using them would record a pass on
-- a bath that had failed. A high-range strip is required, kept with the baths and separate from the
-- food-contact strips, for the same reason the two dilutions are made up in separately labelled
-- containers. Open item 2.
--
-- AND THE CHECK NEEDS SOMEWHERE TO LAND. A daily check with no record is not evidence. The daily
-- check goes on FRM-903 (the fillable daily record) and the monthly strip-down on FRM-901 (the
-- schedule) - the same split SOP-906 already uses. Open item 3, bundled with the FRM-907 basin line.
--
-- Note the correction to 20260901000005, which said only "on FRM-901": a daily check does not belong
-- on a master sanitation SCHEDULE, it belongs on the daily record.
--
-- Body grows 91 -> 92 lines, still 10 Parts.
--
-- GUARDED AGAINST A STALE WRITE: asserts draft, 91 lines, and the fixed-daily wording this
...
begin
  select status,
         ...
         (content->'procedure')::text like '%made up fresh at the start of each production day%'
    into st, lines, v6
  ...
  if lines <> 91 or not v6 then
    raise exception 'FSQM-012 is not the version this migration was written against (% procedure lines, fixed-daily wording present=%). Apply the earlier migrations, or re-derive this one if the row has been edited.', lines, v6;
  end if;
end $$;
...
begin
  select jsonb_array_length(content->'procedure') as lines,
         ...
         (content->'procedure')::text like '%at the start of every production day%' as daily_check,
         (content->'procedure')::text like '%not replaced on a fixed schedule%'     as not_fixed,
         (content->'procedure')::text like '%high-range quat strip%'                as high_range,
         (content->'procedure')::text like '%recorded on **FRM-903**%'              as frm903,
         (content->>'revision_history') like '%three items, all small%'             as open_three,
         (content->>'revision_history') like '%high-range quat test strips%'        as item2,
         -- replaced wording must be gone
         (content->'procedure')::text like '%made up fresh at the start of each production day%' as v6_wording,
  ...
  if r.lines <> 92 or r.parts <> 10 then
    raise exception 'FSQM-012 body wrong shape: % lines, % Parts (expected 92 / 10)', r.lines, r.parts;
  end if;
  if not (r.conditional and r.daily_check and r.not_fixed and r.high_range and r.frm903) then
    raise exception 'FSQM-012 foot bath wording did not land: conditional=%, daily check=%, not fixed=%, high-range=%, FRM-903=%',
      r.conditional, r.daily_check, r.not_fixed, r.high_range, r.frm903;
  end if;
  if r.v6_wording then
    raise exception 'FSQM-012 still carries the fixed-daily foot bath wording.';
  end if;
  if not (r.open_three and r.item2) then
    raise exception 'FSQM-012 open list wrong: three items=%, high-range strip action=%',
      r.open_three, r.item2;
  end if;
  if not (r.ceramic and r.labelled and r.disposable and r.monthly) or r.dangling then
    raise exception 'FSQM-012 regressed an earlier migration: ceramic=%, labelled=%, disposable=%, monthly=%, Form-0010=%',
      r.ceramic, r.labelled, r.disposable, r.monthly, r.dangling;
  end if;
