-- FSQM-012 - break room fittings, the foot bath's strength, and handwash coverage.
-- Closes open items 2, 5, 6(a) and 7 of the seven left by 20260901000003. Four remain.
--
-- Answered by the site 2026-09-01:
--   - the break room DOES have a refrigerator, and a table with seating for all employees;
--   - the foot baths are dosed with Noble Sani-512 at 1:160 - 4.0 fl oz per 5 gallons;
--   - one handwash station covers the entire production floor.
--
-- THE FOOT BATH DILUTION IS THE SUBSTANTIVE ADDITION, AND IT INTRODUCES A HAZARD THE DOCUMENT NOW
-- NAMES. This site uses ONE product at TWO strengths: Sani-512 at 1:512 (about 200 ppm quat) on
-- food-contact surfaces, written into SOP-901 through SOP-906 and FRM-903, and Sani-512 at 1:160
-- in the foot baths. The stronger ratio is correct for a foot bath - it carries heavy organic soil
-- off the floor and that soil consumes quat, so the food-contact ratio would deactivate and leave
-- footwear unsanitized. But two dilutions of one bottle is a confusion route in both directions,
-- and the one that matters is a food-contact surface sanitized at 1:160: that is no longer a
-- no-rinse strength. Part 4 states both ratios together and says plainly that neither substitutes
-- for the other, rather than leaving the floor to infer it.
--
-- WHAT THE ANSWER DID NOT INCLUDE, AND WHY IT STAYS OPEN. A concentration is half a foot bath
-- control; the other half is how often it is changed, because the soil the bath collects is what
-- spends the quat. A correctly dosed bath nobody changes is a tray of dirty water with a number
-- attached to it. Open item 4 (a) asks for an interval, on FRM-901, verified on a test strip the
-- way the SSOPs already verify the food-contact solution.
--
-- AND THE 1:160 FIGURE IS NOT YET LABEL-VERIFIED. The 1:512 food-contact dilution was checked
-- against the actual Sani-512 label on 2026-08-25. This ratio arrived with a bracketed citation of
-- unclear origin, and the site's chemical file still holds only a generated summary rather than the
-- label and SDS. Using a registered sanitizer at a dilution its label does not carry for that use
-- is a label-use problem before it is an SQF problem, so open item 4 (b) asks for the figure to be
-- confirmed against the label - not because it looks wrong, but because it has not been checked and
-- the comparable figure was.
--
-- HANDWASH COVERAGE is recorded as the arrangement plus its basis: one station, immediately inside
-- the floor's only entrance from non-production space. 11.3.2.2 asks for further stations "as
-- required", which is a risk-based judgement, so the document states the judgement and the trigger
-- to revisit it rather than asserting bare compliance.
--
-- Break room: the refrigerator, table and seating close 11.3.5.9 (i), (ii) and (iv)'s refrigeration
-- limb. The SINK (iii) is still absent and the body now says so in the plain present tense rather
-- than as a "shall be equipped" aspiration. Food HEATING (iv) was never stated - the earlier answer
-- described the room's HVAC, which is space heating, not a means of heating food - so it rides
-- along with the sink as open item 2.
--
-- The walk-in rule becomes its own bullet. It was written as a clause of the break-room-shall-be-
-- equipped sentence, which would have disappeared with it; it is a standing rule about where staff
-- food goes and it outlives the gap that prompted it.
--
-- Body grows 85 -> 88 lines, still 10 Parts.
--
-- GUARDED AGAINST A STALE WRITE: asserts draft, 85 lines, the post-20260901000003 soap wording, and
-- the break-room wording this migration replaces. SOP-11.3 still untouched.
...
begin
  select status,
         ...
         (content->'procedure')::text like '%Soap is supplied from a hands-free dispenser%',
         (content->'procedure')::text like '%The break room shall also be equipped with%'
    into st, lines, v3, brk
  ...
  if not v3 then
    raise exception 'FSQM-012 does not carry the 20260901000003 dispenser wording - apply that migration first.';
  end if;
  if lines <> 85 or not brk then
    raise exception 'FSQM-012 is not the version this migration was written against (% procedure lines, break-room wording present=%). It has been edited; re-derive before applying.', lines, brk;
  end if;
end $$;
   ...
   set content = $json$
{
  ...
  "procedure": [
    ...
    "• The **break room** is separate from all food contact and handling zones, is lit, and is ventilated by the site's heating and cooling system. It has **a refrigerator**, and **a table with seating for all employees at one sitting** (11.3.5.9 i, ii, iv). It holds the personal-item lockers and is kept clean and free from waste materials and pests.",
    "• The break room does **not** have a **sink served with hot and cold potable water for washing utensils** (11.3.5.9 iii). The handwash basin in the adjacent lavatory is a handwash basin, not the utensil sink that clause asks for. Whether a means of heating food is provided is not yet recorded (11.3.5.9 iv).",
...
begin
  select jsonb_array_length(content->'procedure') as lines,
         ...
         (content->'procedure')::text like '%stronger than the 1:512 food-contact dilution%' as two_ratios,
         (content->'procedure')::text like '%a refrigerator**, and **a table with seating%'  as breakroom,
         (content->'procedure')::text like '%One station serves the production floor%'       as coverage,
         (content->'procedure')::text like '%not stored in the production walk-in%'          as walkin_rule,
         (content->>'revision_history') like '%four items%'                        as open_four,
         (content->>'revision_history') like '%label-use problem%'                 as label_check,
         -- replaced wording must be gone
         (content->'procedure')::text like '%The break room shall also be equipped with%'    as v3_breakroom,
         -- earlier migrations must not have regressed
         (content->'procedure')::text like '%sanitizing foot baths%'               as foot_baths,
         (content->'procedure')::text like '%laundered by employees at home%'      as laundering,
  ...
  if r.lines <> 88 or r.parts <> 10 then
    raise exception 'FSQM-012 body wrong shape: % lines, % Parts (expected 88 / 10)', r.lines, r.parts;
  end if;
  if not (r.foot_strength and r.two_ratios and r.breakroom and r.coverage and r.walkin_rule) then
    raise exception 'FSQM-012 answers did not all land: foot strength=%, two ratios=%, break room=%, coverage=%, walk-in rule=%',
      r.foot_strength, r.two_ratios, r.breakroom, r.coverage, r.walkin_rule;
  end if;
  if r.v3_breakroom then
    raise exception 'FSQM-012 still carries the "break room shall also be equipped" wording.';
  end if;
  if not (r.open_four and r.label_check) then
    raise exception 'FSQM-012 revision history wrong: four open items=%, label check recorded=%',
      r.open_four, r.label_check;
  end if;
  if not (r.foot_baths and r.laundering and r.soap) or r.dangling then
    raise exception 'FSQM-012 regressed an earlier migration: foot baths=%, laundering=%, soap=%, Form-0010=%',
      r.foot_baths, r.laundering, r.soap, r.dangling;
  end if;
