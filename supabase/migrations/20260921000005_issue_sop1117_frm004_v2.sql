...
-- Every existing field and column id is kept. FRM-004's one entry is an unsubmitted draft; its rows
-- keep their answers and simply show the new columns empty.
--
-- The vacuum sealer (the CCP 2 machine) is not on the register's fixed list. That is the entry's
-- content, added by the owner in the app with "Add equipment" - not a schema change.
...
begin
  ...
  if h <> '2d27895adf57a47dcdca87c83c11484c' then
