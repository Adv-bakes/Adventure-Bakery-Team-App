-- FSQM-009 - Corrective and Preventive Action (CAPA) Program. D-07.
--
-- SQF 2.5.3 Corrective and Preventative Action is a MANDATORY element of the Code and the site has
-- no document against it. Five Minor findings hang off that absence: 2.1.3.3 (no documented process
-- for investigating and resolving complaints aligned to 2.5.3), 2.5.3.1 (no procedure stating the
-- methods and responsibilities), 2.5.3.2 (no records of investigation, root cause and resolution),
-- 2.5.4.4 (audit and inspection findings and their CAPAs not recorded per 2.5.3), and 2.6.3.3
-- (recall and withdrawal root cause investigations and their CAPAs not recorded).
--
-- THE SITE WAS NOT STARTING FROM NOTHING, AND THE DOCUMENT SAYS SO. The remediation plan's build
-- key asserts "No CAPA anywhere in the codebase". That is true of a PROCEDURE and of a REGISTER,
-- but not of records. A read of the live register on 2026-09-02 found four active, fillable forms
-- already capturing root cause and corrective action for their own source:
--
--   FRM-002 Customer Complaint Report            root_cause_sqf_2_5_3_1, corrective_and_preventive_
--                                                action_taken, responsible, target_date, closure_date
--   FRM-205 Supplier Non-Conformance & CAR       root_cause_analysis, corrective_actions_taken,
--                                                preventative_actions, effectiveness verification
--   FRM-702 Non-Conforming Material Hold         associated_scar_number, disposition + justification
--   FRM-908 Glass Breakage Incident Report       root_cause, corrective_preventive_action, and
--                                                car_ref - a corrective-action reference field that
--                                                has pointed at nothing since the day it was written
--
-- So the real gap is a governing procedure and a single register, not the records. Those four forms
-- are therefore KEPT AND REFERENCED, NOT REPLACED. A complaint needs the customer and the lot; a
-- SCAR needs the supplier and the purchase order. Folding all of that into one universal form would
-- have destroyed detail the sources need in order to serve their own clauses.
--
-- ESCALATION IS RISK-BASED, WHICH IS A DELIBERATE CHOICE AND NOT A SOFTENING. Part 3 lists ten
-- triggers that always open a CAPA, and states that a routine on-the-spot correction stays on the
-- form that found it. The alternative - one CAPA per non-conformance - audits more simply but fills
-- the register with trivia, and a register of trivia hides the findings that matter. For a
-- three-person site it would be useless within a quarter. 2.1.3.3 supports the risk-based reading
-- in terms: action is taken "based on the seriousness of the incident and the root cause analysis".
--
-- IT DOES NOT CLAIM THE TWO PROGRAMS IT IS ADJACENT TO. The consultant's evidence against 2.5.4.4
-- reads that the site has not developed an Internal Audit Program, and against 2.6.3.3 that it has
-- not developed a Product Recall and Withdrawal Program. Those are separate deliverables. What this
-- program closes is the RECORDS limb of each: that an audit or inspection finding, and the root
-- cause investigation into a withdrawal or recall, are raised, investigated, actioned, verified and
-- retained through a documented CAPA process. The revision history says this in as many words, so
-- nobody later reads more into the deliverable than it did.
--
-- IT CLOSES AN OPEN ITEM FSQM-022 LEFT YESTERDAY. That program was issued 2026-09-01 saying "the
-- corrective-action half of this program has nowhere to live beyond FRM-913's own columns. D-07
-- (CAPA) is the object findings from any source should be raised into, and until it exists a
-- finding closed on FRM-913 is tracked only on the form that raised it. Task 13.7, the first
-- inspection, depends on it." It now has one.
--
-- IT CREATES NO DANGLING REFERENCES. Every document named in the body was checked against the live
-- register on 2026-09-02, scanning the VALUES rather than the JSON serialization - an escaped
-- newline glues to the next token and defeats a word boundary, which hid an FRM-901 reference once
-- already. Fifteen distinct references: thirteen exist today, and the two that do not (FRM-007,
-- REP-007) are created by the migrations that follow this one. FSQM-018 and FSQM-019 are named
-- EXPLICITLY AS DRAFTS in the Scope rather than cited as though they were approved - which is the
-- precise mistake (Form-0010) that put FSQM-022 in the gap assessment.
--
-- FIVE SITE DECISIONS GO IN revision_history AS AN OPEN BEFORE ACTIVATION LIST, not as placeholders
-- in the body - the convention set by 20260825000005_sop906_mold_hand_wash_ssop.sql. Retention
-- period, who besides the SQF Practitioner may close a CAPA, whether Part 3's ten triggers match
-- how the floor actually runs, what happens when the practitioner is the action owner and there is
-- no independent verifier, and the status of FSQM-018. A question inside an active controlled
-- document is worse than a question somebody owes an answer to.
--
-- procedure[] is 63 lines: 10 numbered Parts, each followed by its sub-bullets. A line
-- starting with a bullet marker renders as a sub-bullet under the Part above it in both the
-- Document tab (SopBodyEditor) and the PDF (sopPdf) - both route through groupProcedureSteps().
--
-- Inserted as status='draft'. Activation is an approval decision and comes as a separate migration
-- once the documents have been read. Idempotent: guarded on sop_number.
...
select
  ...
  $json$
{
  "purpose": "To document the responsibility and methods by which Adventure Bakery, LLC determines, implements and verifies corrective and preventive action — including the identification of the root cause and the resolution of non-compliance with critical food safety limits and deviations from food safety requirements — and to ensure that records of every investigation, root cause analysis, resolution, correction and preventive action are maintained.",
  "scope": "Applies to every non-conformance and every deviation from food safety requirements arising anywhere in the SQF System, whatever raised it: customer and consumer complaints, supplier non-conformances, non-conforming raw material, packaging, work-in-progress, finished product or equipment, non-compliance with a critical food safety limit, findings from internal and external audits and from regulatory inspections, findings from GMP and pre-operational inspections, environmental monitoring results, glass and brittle plastic breakage, and withdrawals and recalls including recall tests.\n\nThis program is the single route every one of those sources leads into. The record is FRM-007 (Corrective & Preventive Action Report) and the register is REP-007 (CAPA Log). The source form that raised the issue keeps its own detail; the CAPA number written back onto it is what ties the two together in both directions.\n\nWHAT THIS PROGRAM DOES NOT COVER. It governs the investigation and the action, not the fate of the product. Physical hold, tagging and disposition of non-conforming material are recorded on FRM-702 (Non-Conforming Material Hold & Tagging Record); the program governing that disposition is FSQM-018, which is an unapproved draft at the date of this revision and takes effect when it is issued. Rework is FSQM-019. Executing a withdrawal or recall is the site's recall and withdrawal procedure; this program covers the root cause investigation and the actions that follow one. The internal audit schedule itself is the internal audit program; this program covers what happens to the findings.",
  "definitions": "Non-conformance — any departure from the SQF Food Safety Code, from this site's own documented programs, from a customer requirement or from a regulatory requirement, whether or not it was corrected on the spot.\n\nCorrection — what is done to fix the thing observed. Corrective action — what is done so it does not happen again. A repeat finding is usually a correction that was never followed by a corrective action.\n\nPreventive action — what is done so that the same cause cannot produce a non-conformance somewhere it has not yet occurred — another line, another product, another shift, another supplier.\n\nRoot cause — the reason the control failed, established by investigation rather than assumed. “Operator error” and “training issue” are not root causes; they are the point at which an investigation stopped.\n\nVerification of effectiveness — the check, made at a stated date after the action is complete, that the non-conformance has actually gone. Completing an action is not verification.\n\nRepeat — the same or substantially the same non-conformance recurring within twelve months, whether or not the earlier one was closed.\n\nCAPA — a Corrective and Preventive Action, opened under Part 3, numbered under Part 4 and recorded on FRM-007.",
  "responsibility": "SQF Practitioner — owns this program. Opens and numbers each CAPA, agrees the root cause, agrees every action with its owner and due date, sets the verification method and date, verifies effectiveness, and closes the CAPA. Reviews REP-007 at least monthly for overdue and repeat items, and reports CAPA performance to management review.\nAll staff — may raise a non-conformance, and shall report one immediately to a supervisor or to the SQF Practitioner. No employee is disadvantaged for raising one.\nAction owners — complete the action they accepted by the date they accepted, and tell the SQF Practitioner before the due date if they cannot.\nSupervisors — ensure containment happens immediately, and that a finding on their shift reaches the SQF Practitioner the same day.\nSenior management — reviews CAPA trends, overdue actions and repeat findings at management review, and provides the resources actions require.",
  "procedure": [
...
begin
  select status, type, revision, sqf_required,
         ...
         -- the load-bearing statements, each tied to the finding it answers
         (content->'procedure')::text like '%based on the seriousness of the incident%' as risk_based,
         (content->'procedure')::text like '%Containment comes before investigation%'   as containment,
  ...
  if r is null then
    raise exception 'FSQM-009 was not created.';
  end if;
  if r.status <> 'draft' or r.type <> 'fsqm' or r.revision <> 'New' or not r.sqf_required then
    raise exception 'FSQM-009 wrong metadata: status=%, type=%, revision=%, sqf_required=%.',
      r.status, r.type, r.revision, r.sqf_required;
  end if;
  if r.lines <> 63 or r.parts <> 10 then
    raise exception 'FSQM-009 body wrong shape: % lines, % Parts (expected 63 / 10).',
      r.lines, r.parts;
  end if;
  if not (r.risk_based and r.containment and r.verification and r.numbering) then
    raise exception 'FSQM-009 is missing a load-bearing statement: risk-based=%, containment=%, verification=%, numbering=%.',
      r.risk_based, r.containment, r.verification, r.numbering;
  end if;
  if not (r.open_list and r.scoped and r.draft_named) then
    raise exception 'FSQM-009 revision history incomplete: open list=%, scoping note=%, FSQM-018 named as draft=%.',
      r.open_list, r.scoped, r.draft_named;
  end if;
end $$;

-- Nothing active is superseded by this program, and nothing may be archived by it. The four source
-- forms it references keep their own records; this asserts the migration touched none of them.
do $$
declare
  n int;
begin
  select count(*) into n from public.sop_documents
   where sop_number in ('FRM-002','FRM-205','FRM-702','FRM-908','FRM-913','SOP-2.1.3','FSQM-022')
     and status <> 'active';
  if n <> 0 then
    raise exception '% of the documents FSQM-009 references are no longer active.', n;
  end if;
end $$;
