...
--
-- SECTION 1 SCANS THE PACK, in finished_goods mode. The pack carries the product name, the lot,
-- the printed date and a UPC, and typing a lot code at a shelf is the entry most likely to be got
-- wrong and the one every later trace exercise depends on. Two pins point AWAY from facts as well
-- as toward them: "Pack retained" would keyword-match pack_size but means what WE kept rather than
-- the label's case configuration, and "Customer" must never receive the pack's brand, which on our
-- own cartons is a bare domain. Discard due is NOT scanned or auto-filled - it is offered as a
-- computed link the filler accepts, because it rests on Part 6's month-end convention and a human
-- belongs between a convention and a record.
--
-- THE UPC IS WORTH A FIELD because it is the only value here that checks itself: a UPC carries a
-- check digit, so a misread is detectable, where a misread lot code is silent. It is a product
-- identity that survives someone typing the product name three different ways across three records.
-- There is nothing yet to look it up AGAINST - the specification library is D-09 - so it is
...
begin
  select status, revision, type, category, sqf_reference, sqf_required,
    ...
    -- The scan pins. Section 1 is scalar, so it reaches the package-label scan through
    -- FormSection.scanLabel rather than the grid path, and in finished_goods mode because
    -- our own carton prints its lot as part of the same block as everything else.
  ...
  if r.sections <> 2 or r.fields <> 20 then
    raise exception 'FRM-703 wrong shape: % sections, % fields (expected 2 / 20).', r.sections, r.fields;
  end if;
  if r.scan_on is distinct from 'true' or r.scan_mode is distinct from 'finished_goods' then
    raise exception 'Section 1 scan is %/% , expected true/finished_goods. The ingredient prompt reads our own carton wrong.',
      coalesce(r.scan_on, 'unset'), coalesce(r.scan_mode, 'unset');
  end if;
  -- Pinned rather than left to the keyword fallback, in both directions. The three positive pins
  -- are what the pack actually carries; the two "none" pins are traps: "Pack retained" is what WE
  -- kept, not the label's pack size, and the brand on our own carton is a bare domain that must
  -- never land in Customer.
  ...
  if r.derive is null
    ...
    raise exception 'discard_due does not derive 30 days from printed_date counting from month end: %.',
