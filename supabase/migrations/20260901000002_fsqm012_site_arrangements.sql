-- FSQM-012 - the site's answers to the twelve OPEN BEFORE ACTIVATION questions.
--
-- FSQM-012 was issued as a draft with twelve statements about the building written as the
-- arrangement the program REQUIRES, and a list in revision_history saying only the site could
-- confirm them. Richard Mercer answered all twelve on 2026-09-01. This replaces the required-
-- arrangement wording with what the facility actually has.
--
-- Nine were confirmations. Three changed the document:
--
--   FOOTWEAR. SOP-11.3 offered dedicated footwear, shoe covers OR a foot bath, which described
--   none of them. The site uses SANITIZING FOOT BATHS, two of them - at the production entrance
--   and at the walkthrough to inventory/packaging. Part 4 now names the actual control.
--
--   LAUNDERING. Aprons and protective clothing are laundered by employees AT HOME, with detergent,
--   hung to dry. 11.3.3.2 permits this but requires the program to say so and to state what is
--   required of it, so Part 4 gains a laundering bullet: laundered after each use or sooner if
--   soiled, washed separately from heavily soiled household laundry, dried and kept clean, carried
--   in a covered bag, presented clean at the start of each shift.
--
--   SENSORY EVALUATION. The draft said tasting is not conducted in food handling zones. It does
--   happen - in the OFFICE adjacent to production, which is outside the zone. Part 8 now describes
--   the practice positively (11.4.1.2 i is satisfied because of where it happens, not because
--   nobody tastes anything) and keeps 11.4.1.4's controls as the standing rule if it ever moves
--   inside the zone.
--
-- THREE ANSWERS REVEALED GAPS, AND THEY ARE NOT SILENTLY ABSORBED:
--
--   1. The protective clothing racks 11.3.3.7 requires DO NOT EXIST. The owner asked where to put
--      them and proposed by the walk-in fridge. The clause wants them adjacent to the personnel
--      access doorway AND the handwashing facilities - which is the production entrance, where the
--      handwash station and foot bath already are. Part 4 states that location; installing the rack
--      is open item 1.
--
--   2. The break room has NO SINK. 11.3.5.9 (iii) asks for one served with hot and cold potable
--      water for washing utensils. The adjacent lavatory has a handwash basin, which is a different
--      fixture for a different purpose and does not satisfy it. Open item 3: install one, or carry
--      it into the 13.6 exemption analysis with a justification.
--
--   3. The break room has NO REFRIGERATION, and the owner asked whether the production walk-in
--      suffices. It does not, and the program now says so in the body rather than only in a note:
--      11.3.5.9 (iv) asks the BREAK ROOM to be equipped so staff can store their own food, and
--      staff food held among ingredients is a contamination and allergen route that 11.3.5.4's
--      separation principle exists to prevent. A small domestic refrigerator closes it.
--
-- TWO THINGS THE ANSWER DID NOT COVER, kept open rather than assumed: the handwash stations were
-- described as having water, towel dispensers and bins, but 11.3.2.3 also asks for liquid soap in a
-- FIXED dispenser, towels in a HANDS-FREE cleanable dispenser, and non-corrosive construction; and
-- one station serves the entire production floor, where 11.3.2.2 asks for stations "at accessible
-- locations throughout food handling and processing areas as required". Open items 4 and 5.
--
-- The foot baths are now the site's footwear control, which raises what a foot bath needs to BE a
-- control - a named sanitizer at a stated dilution, changed on a schedule. Open item 6.
--
-- Twelve questions become seven open items, the first three of which are actions rather than
-- questions. Status stays draft; issuing it is INT-7's job.
--
-- GUARDED AGAINST A STALE WRITE. This replaces `content` wholesale, so it first asserts the row is
-- still the version this migration was written against - draft, 81 procedure lines, and still
-- carrying the v1 three-option footwear wording that this migration replaces. If someone edited it since
-- 20260901000001, the migration RAISES instead of silently discarding that edit. That is the
-- failure recorded in form-schema-migration-stale-tab, and it has cost this project data twice.
--
-- SOP-11.3 is still not touched. It stays active until FSQM-012 is approved and issued.
...
begin
  select status,
         ...
         (content->'procedure')::text like '%passing through a sanitizing foot bath at entry%'
    into st, lines, v1
    from public.sop_documents where sop_number = 'FSQM-012';

  if st is null then
    raise exception 'FSQM-012 does not exist - run 20260901000001 first.';
  end if;
  if st <> 'draft' then
    raise exception 'FSQM-012 is % - this migration edits a draft, not an issued document.', st;
  end if;
  if lines <> 81 or not v1 then
    raise exception 'FSQM-012 is not the version this migration was written against (% procedure lines, v1 footwear wording present=%). It has been edited since 20260901000001; re-derive against the current body before applying.', lines, v1;
  end if;
end $$;
   ...
   set content = $json$
{
  ...
  "procedure": [
    ...
    "• Each station shall also provide **liquid soap in a fixed dispenser** and **paper towels in a hands-free, cleanable dispenser**, and shall be of stainless steel or a similar non-corrosive material (11.3.2.3).",
...
begin
  select jsonb_array_length(content->'procedure') as lines,
         ...
         (content->'procedure')::text like '%office adjacent to the production area%' as tasting,
         ...
         (content->>'revision_history') like '%SITE ARRANGEMENTS CONFIRMED%'    as confirmed,
         (content->>'revision_history') like '%seven items remain%'             as open_seven,
         -- the wording this migration replaces must be gone
         (content->'procedure')::text like '%passing through a sanitizing foot bath at entry%' as old_footwear,
         (content->'procedure')::text like '%Form-0010%'                        as dangling_ref
    into r
    from public.sop_documents where sop_number = 'FSQM-012';

  if r.lines <> 85 or r.parts <> 10 then
    raise exception 'FSQM-012 body wrong shape after update: % lines, % Parts (expected 85 / 10)',
      r.lines, r.parts;
  end if;
  if not (r.foot_baths and r.laundering and r.tasting and r.racks and r.walkin_rule) then
    raise exception 'FSQM-012 site answers did not all land: foot baths=%, laundering=%, tasting=%, racks=%, walk-in rule=%',
      r.foot_baths, r.laundering, r.tasting, r.racks, r.walkin_rule;
  end if;
  if not (r.confirmed and r.open_seven) then
    raise exception 'FSQM-012 revision history wrong: confirmations=%, seven open items=%',
      r.confirmed, r.open_seven;
  end if;
  if r.old_footwear then
    raise exception 'FSQM-012 still carries the three-option footwear wording the site replaced.';
  end if;
