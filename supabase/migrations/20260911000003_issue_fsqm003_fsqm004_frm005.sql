-- D-01 / D-02 - issue FSQM-003, FSQM-004 and FRM-005. Active, GJM, effective 2026-09-11.
--
...
begin
  select
    ...
    (select count(*) from public.sop_documents
      where sop_number in ('FSQM-003', 'FSQM-004', 'FRM-005')
        and lower(content::text) ~ 'diana|gabriela|samboni|juncos|christina')                                       as named
  into r;
...
update public.sop_documents
   set content = jsonb_set(content, '{revision_history}', to_jsonb(
         replace(
           ...
           $iss$ISSUED 2026-09-11, approved GJM (Senior Site Management), effective 2026-09-11.$iss$)
