# -*- coding: utf-8 -*-
"""Issue FSQM-017: active, approved GJM, effective 2026-09-10.

Closes 2.5.2.1 and 2.5.2.2 in force, and 2.5.1.1 in part - Part 2's analytical limb cannot be
met until the environmental monitoring programme exists, and Part 3 has nothing to bite on until
a food safety plan establishes critical limits. Both are stated on the document rather than
implied, and that partial closure is what the workbook should record.

Also sets first_due_on on the annual programme review. It is evidenced by a revision of
FSQM-017, and issuing is not a review - without an anchor the activity would read as never
recorded and nag from the day the programme went live.

Refuses to overwrite the migration file.

Usage:  python scripts/build-issue-fsqm017.py
"""
import io, json, os, sys

sys.stdout.reconfigure(encoding="utf-8", errors="replace")

OUT = "supabase/migrations/20260910000019_issue_fsqm017.sql"
if os.path.exists(OUT):
    raise SystemExit("%s already exists - refusing to overwrite an applied migration." % OUT)

SETTLED = """SETTLED AT ISSUE — 2026-09-10:
...
WHAT THIS CLOSES. 2.5.2.1 and 2.5.2.2 are in force from this date. 2.5.1.1 is met in part - limb iii by Part 4, limbs i and ii as far as a site with no analysis and no food safety plan can meet them, with both gaps stated on the document. Recording that as a full closure would be closing a Mandatory clause on evidence that does not exist."""

SQL = """-- D-18 - issue FSQM-017 Validation and Verification Program. Active, GJM, effective 2026-09-10.
--
...
s = json.dumps("\n\n" + SETTLED, ensure_ascii=False)
assert "$s17$" not in s
SQL = SQL.replace("__SETTLED__", "$s17$" + s[1:-1].replace("\\n", "\n").replace('\\"', '"') + "$s17$")
